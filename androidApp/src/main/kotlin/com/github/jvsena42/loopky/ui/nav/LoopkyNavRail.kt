 ...
 *
 * Collapsed, and with nothing to expand it: each item already carries its own one-word label under
 * its icon, so the drawer the toggle opened showed the same four words a second time, in a panel
 * drawn over the content the reader had just navigated to.
 */
@Composable
fun LoopkyNavRail(
    ...
    WideNavigationRail(
        ...
        LoopkyTab.entries.forEach { tab ->
            ...
            WideNavigationRailItem(
                ...
                label = { Text(tabLabel) },
                // The icon-over-label form, permanently: this rail no longer expands, and the
                // expanded item puts its label beside the icon in a drawer that is never open.
                railExpanded = false,
